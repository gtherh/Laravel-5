...
@section('page_title', __('Create :x', ['x' => __('Tag')]))
@section('css')
    <!--custom css-->
        ...
        <div class="card">
            <div class="card-header">
                <h5> <a href="{{ route('tags.index') }}">{{ __('Tags') }} </a>
                    >>{{ __('Create :x', ['x' => __('Tag')]) }}</h5>
            </div>
            <div class="card-block table-border-style">
                <div class="row form-tabs">
                    <form action="{{ route('tags.store') }}" method="post" id="brandAdd" class="form-horizontal"
                            ...
                            <div class="tab-pane fade show active form-con" id="home" role="tabpanel"
                                ...
                                <div class="row">
                                    <div class="col-sm-9">
                                        ...
                                        <div class="form-group row">
                                            <label for="name" class="control-label require ps-3">{{ __('Name Arabic ') }}
                                            </label>
                                            <div class="col-sm-12">
                                                <input type="text" placeholder="{{ __('Name') }}"
                                                    class="form-control form-width inputFieldDesign" id="name"
                                                    name="name_ar" value="{{ old('name->en') }}" required
                                                    oninvalid="this.setCustomValidity('{{ __('This field is required.') }}')">
                                            </div>
                                        </div>
