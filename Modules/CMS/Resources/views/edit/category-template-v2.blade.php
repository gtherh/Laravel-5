...
$allTags = \Modules\CMS\Service\Homepage::getTagList()->pluck('name', 'id');
    ...
    <div class="card-body">
        <form action="{{ route('builder.update', ['id' => '__id']) }}" data-type="component" method="post"
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-8">
                    <select type="text" class="col-sm-12 form-control category_type select3" name="category_type">
            ...
            {{-- tags [] --}}
            <div
                class="form-group row cats">
                <label class="col-sm-3 control-label">{{ __('Tags') }}</label>
                <div class="col-sm-8">
                    <select type="text" class="form-control select2" name="tags[]" multiple>
                        @if ($component && is_array($component->tags))
                        @foreach ($component->tags as $selected)
                        @if (isset($allTags[$selected]))
                        <Option selected value="{{ $selected }}">{{ $allTags[$selected] }}
                        </Option>
                        @php
                        unset($allTags[$selected]);
                        @endphp
                        @endif
                        @endforeach
                        @endif
                            ...
                            <div class="form-group row product-count">
                                ...
                                <div class="col-sm-6 pr-1">
                                    <input type="number" class="form-control product-row inputFieldDesign"
                                        placeholder="{{ __('Rows') }}" value="{{ $component ? $component->row : '' }}"
                                        min="1" max="12" name="row">
                                </div>
                                <div class="col-sm-6 pl-1">
                                    <input type="number" class="form-control product-col inputFieldDesign"
                                        placeholder="{{ __('Columns') }}"
                                        value="{{ $component ? $component->column : '' }}" min="1" max="12"
                                        name="column">
                                </div>
                                <input type="hidden" name="max" value="{{ $component ? $component->max : '' }}"
                                    class="product-quantity">
                            </div>
                            <div class="mt-2">
                                <span class="badge badge-info mt-1">{{ __('Example') }}</span>
                                <small class="mt-1 ml-1">{{ __('Total categories will be rows x columns') }}</small>
                            </div>
