...
class GatewayController extends Controller
{
    ...
    public function paymentGateways(Request $request)
    {
        ...
        $getway = 'clickpay';
        //  add getway to request 
        $request->merge(['gateway' => $getway]);
        $request->query->add(['gateway' => $getway]);
        $request->query->add(['integrity' => getIntegrityKey()]);
        $request->query->add(['code' => techEncrypt($purchaseData->code)]);
        $response = $this->makePayment($request);
    ...
    public function paymentCallback(Request $request)
    {
        ...
        try {
            ...
            return redirect(route('gateway.failed', withOldQueryIntegrity()))->withErrors($e->getMessage());
        }
    }
