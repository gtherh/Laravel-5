...
@section('logo', asset(config('clickpay.logo')))

@section('gateway', config('clickpay.name'))
...
@section('content')
    ...
    <form class="pay-form needs-validation"
        action="{{ route('gateway.complete', withOldQueryIntegrity(['gateway' => config('clickpay.alias')])) }}"
