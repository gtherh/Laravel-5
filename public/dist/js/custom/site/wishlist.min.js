...
$(document).on("click", ".wishlist", function () {
    ...
    setTimeout(() => {
        ...
        $.ajax({
            ...
            type: "POST",
            dataType: "JSON",
            data: { product_id: item_id, _token: token },
            success: function (data) {
                (document.cookie = "product_id=; Max-Age=-99999999;"),
                    $(svg).hasClass("text-gray-10")
                        ? ((svg = svg.replace(
                              "text-gray-10",
                              "color_fill svg-bg"
                          )),
                          $("#totalWishlistItem").text(
                              Number($("#totalWishlistItem").text()) + 1
                          ),
                          $("#totalWishlistItem").addClass("w-4 h-4"))
                        : $(svg).hasClass("color_fill")
                        ? ((svg = svg.replace(
                              "color_fill svg-bg",
                              "text-gray-10"
                          )),
                          ...
                          0 == $("#totalWishlistItem").text() &&
                              ($("#totalWishlistItem").text(""),
                              $("#totalWishlistItem").removeClass("w-4 h-4")))
                        : wishlist.find(".fa-heart-o").length
                        ? (wishlist
                              .find("i")
                              .removeClass("fa-heart-o text-black"),
                          wishlist
                              .find("i")
                              .addClass("fa-heart text-green-500"),
                          wishlist
                              .find("span")
                              .text(jsLang("Remove from wishlist")),
                          $("#totalWishlistItem").text(
                              Number($("#totalWishlistItem").text()) + 1
                          ))
                        : wishlist.hasClass("add-wishlist")
                        ? (wishlist.addClass(
                              "remove-wishlist primary-bg-color"
                          ),
                          ...
                          $("#totalWishlistItem").addClass("w-4 h-4"))
                        : wishlist.hasClass("remove-wishlist")
                        ? (wishlist.addClass("add-wishlist"),
                          wishlist.removeClass(
                              "remove-wishlist primary-bg-color"
                          ),
                          $("#totalWishlistItem").text(
                              Number($("#totalWishlistItem").text()) - 1
                        ...
                        : (wishlist
                              .find("i")
                              .removeClass("fa-heart text-gray-10"),
            ...
            complete: function () {
                wishlist.html(svg), (wishlistClick = 0);
            },
        });
}),
    $(".shop-search-icon").on("click", function () {
        window.innerWidth <= 624 &&
            ($(".search-in-store").toggle(), $(".shop-menu").toggle());
    });
var clickCount = 0;
