...
{
    use ProductTrait, ModelTrait, hasFiles, SoftDeletes, HasTranslations;
    ...
    protected $fillable = [
        ...
        'sale_from', 'sale_to', 'featured', 'type', 'manage_stocks', 'total_stocks', 'menu_order', 'name_old'
    ];
    ...
    {
        $category_id = $this->productCategory?->category_id ?? null;
...
// dd($price, $tempPrice);
    ...
    public function getAddress($addressId = null)
    {
        ...
        if ($calculateTaxSetting == 'customer billing address' && !isset(request()->ship_different) || $calculateTaxSetting == 'customer billing address' && is_numeric($addressId) || $calculateTaxSetting == 'customer billing address' && is_object($addressId)) {
            ...
            $calculateTaxSetting == 'customer shipping address' && is_object($addressId) && isset(request()->ship_different) && request()->ship_different == 'on'
    ...
    public function shipping($params = [])
    {
        ...
        if (!$this->isVirtual() && !$this->isExternalProduct() && !$this->isGroupedProduct()) {
            ...
            if (
                in_array($calculateShippingSetting, ['billing_address', 'force_billing_address']) ||
