...
class ProductSearchFilter extends Filter
{
    ...
    public function brands($value)
    {
        ...
        return $this->query->whereHas('brand', function ($q) use ($brandArray) {
            return $q->whereIn('name->' . app()->getLocale(), $brandArray);
        });
    }
    ...
    public function tag($value)
    {
        ...
        $tags = Tag::whereIn('name' . app()->getLocale(), $tagArray)->pluck('id')->toArray();

        return  $this->query->WhereHas('productTag', function ($query) use ($tags) {
    ...
    public function categories($value)
    {
        ...
        $category = Category::where('name->' . $locale, $value)->first();
        ...
        if (isset($category->childrenCategories)) {
            ...
            $childTwo = Category::whereIn('parent_id', array_keys($children))->pluck('name')->toArray();
        ...
        return $this->query->whereHas('category', function ($q) use ($categoryArray) {
            ...
            return $q->whereIn('name->' . app()->getLocale(), $categoryArray);
        });
    }
